...
<template>
  <div
    ...
    <div class="flex flex-row w-full flex-nowrap items-center gap-2">
      <NuxtLink aria-label="Home"
        ...
        to="/">
        <NuxtImg alt="Bodega" loading="lazy" src="bodega.png"
           />
      </NuxtLink>
      <NuxtLink aria-label="Categories" exactActiveClass="bg-black dark:bg-white text-white dark:text-black"
      ...
      <NuxtLink aria-label="Favorites" exactActiveClass="bg-black dark:bg-white text-white dark:text-black"
        class="font-semibold cursor-pointer px-4 rounded-full hover:bg-black hover:dark:bg-white h-12 items-center justify-center hover:text-white hover:dark:text-black transition active:scale-95 lg:flex hidden"
        to="/favorites">
        Favorites
      </NuxtLink>
      <NuxtLink aria-label="Categories" exactActiveClass="!bg-black/10 dark:!bg-white/30"
      ...
      <NuxtLink aria-label="Favorites" exactActiveClass="!bg-black/10 dark:!bg-white/30"
        class="lg:hidden flex items-center justify-center min-w-12 min-h-12 rounded-full bg-black/5 dark:bg-white/15 hover:bg-black/10 hover:dark:bg-white/20 transition active:scale-95"
        to="/favorites">
          ...
          <div @click="suggestionMenu = true" class="flex w-full items-center gap-4">
            ...
            <div class="flex w-full">
              <input
                ...
                type="text" v-model="searchQuery" @keyup.enter="search"
                :placeholder="route.query.category ? `Search in ${route.query.category}...` : 'Search...'" />
              <div v-if="searchQuery || suggestionMenu" @click.stop="clearSearch"
                class="flex items-center justify-center cursor-pointer transition-all">
                <UIcon v-if="!isLoading" class="text-black dark:text-white" name="i-iconamoon-close-circle-1-fill"
                  size="24" />
      ...
      <button @mouseup="cartModal = !cartModal"
        ...
        <span v-if="cart.length" class="absolute top-1 right-1 flex h-[18px] w-[18px]">
          <span
            class="animate-ping absolute inline-flex h-full w-full rounded-full bg-alizarin-crimson-400 opacity-75"></span>
          <span
  ...
  <div v-if="suggestionMenu" ref="onClickOutsideRef"
    ...
    <div class="max-h-[calc(100vh-72px)] lg:max-h-[calc(100vh-80px)] overflow-auto">
      ...
      <div v-else class="mx-auto p-3 lg:p-4 max-w-screen-2xl">
        ...
        <div
          ...
          <NuxtLink @click="suggestionMenu = false" :to="`/product/${product.slug}-${product.sku.split('-')[0]}`"
            v-for="(product, i) in searchResults" :key="i" class="group select-none">
      ...
      <div v-if="searchQuery && !isLoading && searchResults.length"
